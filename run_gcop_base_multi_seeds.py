...
# Runs the OLSR Blackhole/GCOP simulation with 10 different RNG seeds and prints
# a summary table comparing PDR across the four phases:
...
# nNodes-1 routing-table entries), that seed is discarded and the next candidate
# seed is tried instead, until 10 successful runs are collected.
...
#     2. ./ns3 configure
#     3. ./ns3 build
#        (--enable-examples is NOT required when the program lives in scratch/.
#         It is only needed if you place the file under src/olsr/examples/ and
#         register it via CMakeLists.txt.)
# """
...
#     # Standard ns-3 form: ./ns3 run "<program> --arg=val ..." --no-build
...
#     # Find the LAST RESULT line (in case multiple lines pollute the output)
#     result = None
...
#             reason = "NO_ATTACKER (no node in [3..N-1] is a 1-hop neighbor of Node 0)"
...
#     # Pre-build once so subsequent runs use --no-build
...
-------
    --ns3            Path to the `ns3` wrapper script (default: ./ns3)
    --program        Name of the simulation program (default: gcopBaseSimulation)
    --num-seeds      How many successful runs to collect (default: 10)
    --max-attempts   How many seeds to try in total before giving up (default: 100)
    --start-seed     First seed to try (default: 1)
    --output-csv     Optional path for a CSV dump of the results
...
PRE-REQUISITES
--------------
    1. Place gcopBaseSimulation.cc in scratch/ in your ns-3 root.
    2. ./ns3 configure
    3. ./ns3 build
       (--enable-examples is NOT required when the program lives in scratch/.
        It is only needed if you place the file under src/olsr/examples/ and
...
def print_skip_log(skipped: list):
    if not skipped:
        return
    print("Seeds skipped (no full connectivity / errors):")
    for s in skipped:
        reason = s.get("STATUS", "?")
        if reason == "NO_CONNECTIVITY":
            reason = (f"NO_CONNECTIVITY (min routes={s.get('minRoutes','?')}"
...
def main():
    ...
    ap.add_argument("--ns3", default="./ns3",
                    help="Path to the ns3 wrapper script")
    ap.add_argument("--program", default="gcopBaseSimulation",
                    help="Simulation program name (without .cc)")
    ap.add_argument("--num-seeds", type=int, default=10,
                    help="How many *successful* runs to collect")
    ap.add_argument("--max-attempts", type=int, default=100,
