...
/*
 * Watchdog-OLSR Evaluation Harness - HIGHWAY VARIANT
 * ===================================================
 *
 * Variant of olsr-watchdog-eval.cc that places nodes in a long, narrow
 * highway-shaped corridor (1000m x 200m by default, longer radio range)
 * matching the topology described by Baiad et al. (IEEE IWCMC 2014),
 * the paper the OlsrWatchdogDefense implementation is based on.
 *
 * Why a separate file: the square 1000x1000m random topology used in the
 * default eval harness produces sparse placements where any single node
 * may be the only bridge between two halves of the network. When the
 * watchdog blacklists such a bridge (rightly or wrongly), the network
 * partitions and PDR collapses for reasons unrelated to the defense's
 * decision quality. The highway shape gives every pair of nodes plenty
 * of route redundancy along the corridor, so isolating one node leaves
 * many alternative paths intact - which is the regime the original
 * watchdog paper assumed.
 *
 * Everything else (4-phase timeline, FlowMonitor wiring, CSV output,
 * defense parameters) is identical to olsr-watchdog-eval.cc, so the
 * results are directly comparable when the topology shape is the only
 * difference.
 *
 * Topology defaults (overridable via CLI):
 *   areaLength=1000m, areaWidth=200m, radioRange=300m, numNodes=50
 *
...
{
  // Topology - highway scenario (per Baiad et al. 2014, Table II).
  // The original VANET-OLSR paper modelled a freeway: a long, narrow strip
  // with high node density and longer radio range. This shape gives every
  // pair of nodes plenty of route redundancy along the highway, so isolating
  // one bridge does not partition the network.
  uint32_t numNodes       = 50;
  double   areaLength     = 1000.0;    // meters (along the highway)
  double   areaWidth      =  200.0;    // meters (across the highway)
  double   radioRange     = 300.0;     // meters (per the paper)
...
//
// Highway variant: places nodes uniformly in a `length` x `width` rectangle
// (not a square). For freeway-like scenarios width is much smaller than
// length, which gives every pair of nodes high route redundancy along the
// highway corridor.
static std::vector<Vector>
PlaceNodesConnected (uint32_t numNodes, double length, double width,
                     double range,
  ...
  for (uint32_t attempt = 0; attempt < maxAttempts; ++attempt)
    {
      ...
      for (uint32_t i = 0; i < numNodes; ++i)
        {
          pos.emplace_back (rng->GetValue (0.0, length),
                            rng->GetValue (0.0, width),
                            0.0);
        }
                  ...
                  const std::vector<uint32_t>& attackerIds)
{
  ...
  std::cout << "  Area            : " << cfg.areaLength << " x " << cfg.areaWidth << " m (highway)\n";
              ...
              const std::vector<uint32_t>& attackerIds)
{
  ...
  if (needHeader)
    {
      out << "seed_used,num_nodes,area_length,area_width,range,"
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("areaLength", "Length of highway corridor (m)", cfg.areaLength);
  cmd.AddValue ("areaWidth",  "Width of highway corridor (m)",  cfg.areaWidth);
  ...
  cfg.positions = PlaceNodesConnected (cfg.numNodes,
                                       cfg.areaLength, cfg.areaWidth,
